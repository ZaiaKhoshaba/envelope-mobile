...
async function registerPushToken(authToken) {
  try {
    ...
    // Never register from inside Expo Go. A push token issued there belongs to
    // the Expo Go app, so the backend delivering to it made Tend's alerts
    // arrive wearing Expo Go's name and icon — and since tokens accumulate per
    // user, every dev session ever run added another one.
    if (Constants.executionEnvironment === "storeClient") {
      console.log("[notifications] Running in Expo Go — not registering for push");
      return null;
    }
...
export function usePushNotifications() {
  ...
  const { setPendingSpend, bankBalance } = useBudget();
  ...
  // Same reason: the tap listener is registered once, so it needs a ref rather
  // than the value captured when it was set up.
  const bankConnectedRef = useRef(bankBalance != null);
  useEffect(() => { bankConnectedRef.current = bankBalance != null; }, [bankBalance]);
  ...
  useEffect(() => {
    ...
    responseListener.current = Notifications.addNotificationResponseReceivedListener(response => {
      ...
      try {
        // ── Payday reminder ──────────────────────────────────────────────────
        // With a bank connected the pay arrives by itself and is already in the
        // balance, so there is nothing to type in: the useful screen is the
        // transaction list, where that pay can be split across the envelopes in
        // one tap. Without a bank, open Add income with the amount filled in.
        if (data?.type === "payday") {
          if (bankConnectedRef.current) { router.push("/transactions"); return; }
          const amount = data.amount ? encodeURIComponent(String(data.amount)) : "";
          router.push(`/add-income${amount ? `?prefill=${amount}` : ""}`);
          return;
        }
