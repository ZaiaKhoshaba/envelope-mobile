...
export function computeTotals(envelopes, transactions, bankBalance = null) {
  // Money moved between the customer's own connected accounts is neither income
  // nor spending — they have exactly as much as before. Counting either leg
  // would inflate income and drain an envelope for money that was never spent.
  const isTransfer = (t) => !!t.transfer || t.kind === "transfer";
...
/* -----------------------------------------------------------
   Spending waiting to come out of an envelope
   Shared by the home screen and the transactions list, so the
   "Spending to sort" total can never disagree with the list.
----------------------------------------------------------- */

// A bank transaction counts as history only if it happened before the bank was
// connected: that money had already left the balance the user divided into
// envelopes. Everything since is live spending that must come out of one.
export function isHistoricalTx(t, bankConnectedAt) {
...
// How much has left the bank but not yet been taken out of an envelope.
// Transfers between the customer's own accounts are not spending.
export function unsortedSpending(transactions, bankConnectedAt) {
  let total = 0;
  let count = 0;
  for (const t of transactions || []) {
    if (!t || t.transfer || t.kind !== "spend") continue;
    if (t.allocated || isHistoricalTx(t, bankConnectedAt)) continue;
    const used = (t.allocations || []).reduce((s, a) => s + (a.used || 0), 0);
    const left = Math.abs(Number(t.amount) || 0) - used;
    if (left > 0.004) { total += left; count++; }
  }
