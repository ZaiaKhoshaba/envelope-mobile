...
// RevenueCat's Test Store — a synthetic store that lets purchases be exercised
// end to end without any App Store / Play products. Used in development only.
...
export function apiKey() {
  // __DEV__ is false in release builds, so the Test Store key can never ship to
  // customers — a release binary always uses the real store key.
  if (__DEV__ && TEST_KEY) return TEST_KEY;
  return Platform.OS === "ios" ? IOS_KEY : ANDROID_KEY;
}

/** True when purchases are running against the synthetic Test Store. */
export function isTestStore() {
  return !!(__DEV__ && TEST_KEY);
}
...
export async function configure(appUserId) {
  ...
  try {
    if (!configured) {
      // Verbose native logs while developing — they're the only place the
      // purchase pipeline is visible (Metro's console doesn't show them).
      if (__DEV__ && Purchases.setLogLevel && Purchases.LOG_LEVEL?.DEBUG) {
        try { Purchases.setLogLevel(Purchases.LOG_LEVEL.DEBUG); } catch { /* ignore */ }
      }
